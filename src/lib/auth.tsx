...
type AuthCtx = {
  // loading = solo “resolviendo sesión”, NO perfil
  loading: boolean;
  session: any | null;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // evita ejecuciones concurrentes de resync (focus + visibility + interval)
  const resyncLockRef = useRef(false);

  // dedupe: evita cargar perfil en paralelo
  ...
  // helper: carga perfil sin bloquear nada
  const loadProfileInBackground = useCallback(
    (user: SbUser) => {
      ensureProfile(user)
        .then((p) => safe(setProfile, p))
        .catch(() => {
          // silencioso: no “rompe” la sesión
        });
    },
    [ensureProfile, safe]
  );
  ...
  const refresh = useCallback(
    async (opts?: { hard?: boolean }) => {
      ...
      try {
        const { data: sess, error: sErr } = await withTimeout(supabase.auth.getSession(), 4500);
        if (sErr) throw new Error(sErr.message);

        const s = sess.session ?? null;
        safe(setSession, s);

        // ✅ ya sabemos si hay sesión => soltamos loading
        ...
        loadProfileInBackground(s.user as SbUser);
      } catch (e: any) {
  ...
  const resyncOnFocus = useCallback(async () => {
    ...
    try {
      ...
      // refresca solo si expira pronto
      const expMs = (s.expires_at ?? 0) * 1000;
      const msLeft = expMs - Date.now();
      if (msLeft < 2 * 60 * 1000) {
        const { data: refreshed } = await supabase.auth.refreshSession();
        const s2 = refreshed.session ?? null;
        if (s2) safe(setSession, s2);
      }

      // cargar perfil si falta (background)
  ...
  }, [loadProfileInBackground, profile, safe]);
  ...
  useEffect(() => {
    ...
    const { data: sub } = supabase.auth.onAuthStateChange((_evt, sess) => {
      ...
      // ✅ NO await: perfil en background
      loadProfileInBackground(sess.user as SbUser);
    });
  ...
  }, [loadProfileInBackground, refresh, resyncOnFocus, safe]);
