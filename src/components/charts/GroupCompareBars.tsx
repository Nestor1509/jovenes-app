...
function fmtMinutesValue(v: number) {
...
// ticks compactos (eje)
function fmtMinutesTick(v: number) {
  ...
  return (
    <div className="w-full" style={{ height: 320 }}>
      <ResponsiveContainer width="100%" height="100%" minHeight={320}>
        <BarChart data={data} margin={{ top: 10, right: 18, left: 0, bottom: 22 }}>
          <defs>
          ...
          <YAxis
            ...
            axisLine={{ stroke: "rgba(255,255,255,0.10)" }}
            width={46}
            label={{ value: "Capítulos", angle: -90, position: "insideLeft", fill: "rgba(255,255,255,0.55)" }}
          />
          ...
          <YAxis
            ...
            orientation="right"
            domain={[0, maxMin || 0]}
            tickFormatter={(v) => fmtMinutesTick(Number(v))}
            tick={{ fontSize: 12, fill: "rgba(255,255,255,0.65)" }}
            tickLine={false}
            axisLine={{ stroke: "rgba(255,255,255,0.10)" }}
            width={46}
            label={{ value: "Oración", angle: 90, position: "insideRight", fill: "rgba(255,255,255,0.55)" }}
          />
          ...
          <Tooltip
            ...
            formatter={(value: any, name: any) => {
              if (name === "Capítulos" || name === "lectura") return [fmtChapters(Number(value)), "Capítulos"];
              return [fmtMinutesValue(Number(value)), "Oración"];
            }}
          />
          ...
          <Legend
            ...
            height={28}
            formatter={(value) => (
              <span style={{ color: "rgba(255,255,255,0.75)", fontSize: 12 }}>{value}</span>
            )}
          />
