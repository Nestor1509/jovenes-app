  ...
  return (
    <div
      className={cn(
        ...
        // ✅ min-w-0 ayuda a truncar en flex/grid SIN recortar sombras
        "min-w-0",
        className
      )}
      {...props}
...
export function Card({
  ...
  return (
    <motion.div
      ...
      className={cn(
        // ✅ padding responsive + NO overflow-hidden (para no cortar glow)
        "glass rounded-2xl p-4 sm:p-5 shadow-glow transition will-change-transform",
...
export function Select({ className, ...props }: React.SelectHTMLAttributes<HTMLSelectElement>) {
  return (
    <select className={cn("input pr-8 transition-colors hover:border-white/20 focus:border-amber-400/40", className)} {...props} />
  );
}
...
export function Stat({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="glass rounded-2xl p-4 shadow-soft min-w-0">
  ...
  return (
    <span
      ...
      className={cn(
        ...
        // ✅ NO romper layout + truncar si hace falta
