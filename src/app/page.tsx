"use client";

import Link from "next/link";
import { useEffect, useMemo, useState } from "react";
import { supabase } from "@/lib/supabaseClient";
import { Container, Card, Title, Subtitle, Button, PageFade } from "@/components/ui";
import { Sparkles, ShieldCheck, ArrowRight, Trophy, ClipboardList, BarChart3 } from "lucide-react";

function traducirError(msg: string) {
  const m = (msg ?? "").toLowerCase();
  if (m.includes("invalid login")) return "Correo o contraseña incorrectos.";
  if (m.includes("email not confirmed")) return "Tu correo no está confirmado (revisa tu email).";
  if (m.includes("jwt")) return "Tu sesión expiró. Inicia sesión de nuevo.";
  if (m.includes("rate limit")) return "Demasiados intentos. Espera un momento y vuelve a intentar.";
  return "Ocurrió un error. Intenta de nuevo.";
}

function QuickLink({
  href,
  title,
  icon,
  disabled,
}: {
  href: string;
  title: string;
  icon: React.ReactNode;
  disabled?: boolean;
}) {
  const cls =
    "flex items-center justify-between gap-3 rounded-xl border px-4 py-4 text-sm transition " +
    (disabled
      ? "border-white/10 bg-black/20 opacity-50 cursor-not-allowed"
      : "border-white/10 bg-black/20 hover:bg-white/5 hover:border-white/15");

  const content = (
    <div className={cls} aria-disabled={disabled}>
      <div className="flex items-center gap-3">
        <div className="h-9 w-9 rounded-xl border border-white/10 bg-white/5 grid place-items-center">
          {icon}
        </div>
        <div className="font-medium text-white/85">{title}</div>
      </div>
      <ArrowRight size={18} className="opacity-60" />
    </div>
  );

  if (disabled) return content;

  return (
    <Link href={href} prefetch className="block">
      {content}
    </Link>
  );
}

export default function Home() {
  const [busy, setBusy] = useState(false);
  const [sessionEmail, setSessionEmail] = useState<string | null>(null);
  const [msg, setMsg] = useState("");

  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => {
      setSessionEmail(data.session?.user.email ?? null);
    });

    const { data: sub } = supabase.auth.onAuthStateChange((_event, session) => {
      setSessionEmail(session?.user.email ?? null);
    });

    return () => sub.subscription.unsubscribe();
  }, []);

  async function signInGoogle() {
    setMsg("");
    try {
      setBusy(true);
      const { error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo: window.location.origin + "/auth/callback",
        },
      });
      if (error) setMsg(traducirError(error.message));
    } finally {
      setBusy(false);
    }
  }

  const hasSession = useMemo(() => !!sessionEmail, [sessionEmail]);

  return (
    <Container>
      <div className="mb-6 flex items-center gap-4">
        <div className="h-14 w-14 rounded-3xl bg-white/5 border border-white/10 grid place-items-center text-lg font-semibold">
          MA
        </div>
        <div>
          <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight">Ministerio Águilas</h1>
          <p className="text-sm text-white/70">Casa de Dios Cruzada Cristiana</p>
        </div>
      </div>

      <PageFade>
        <div className="grid gap-6 md:grid-cols-2 items-start">
          <Card className="relative overflow-hidden">
            <div className="absolute -top-24 -right-24 h-56 w-56 rounded-full bg-white/10 blur-2xl" />
            <div className="absolute -bottom-24 -left-24 h-56 w-56 rounded-full bg-white/10 blur-2xl" />

            <div className="flex items-center gap-2">
              <Sparkles size={18} className="opacity-80" />
              <Title>Bienvenido</Title>
            </div>
            <Subtitle>Lleva un registro sencillo de tu lectura bíblica y tu tiempo de oración.</Subtitle>

            {/* ✅ Ahora estos son “botones” que navegan */}
            <div className="mt-5 grid gap-3 text-sm text-white/80">
              <QuickLink
                href="/reporte"
                title="Reporte diario"
                icon={<ClipboardList size={18} className="opacity-85" />}
                disabled={!hasSession}
              />
              <QuickLink
                href="/mis-estadisticas"
                title="Estadísticas personales"
                icon={<BarChart3 size={18} className="opacity-85" />}
                disabled={!hasSession}
              />
              {/* 🔥 Este es el nuevo: Ranking */}
              <QuickLink
                href="/ranking"
                title="Ranking del grupo"
                icon={<Trophy size={18} className="opacity-85" />}
                disabled={!hasSession}
              />
            </div>

            {sessionEmail && (
              <div className="mt-5 text-sm text-white/70">
                Sesión iniciada: <strong className="text-white">{sessionEmail}</strong>
              </div>
            )}
          </Card>

          <Card>
            <Title>{sessionEmail ? "Listo para continuar" : "Iniciar sesión"}</Title>
            <Subtitle>
              {sessionEmail
                ? "Usa el menú superior para Reporte, Mis estadísticas, Ranking, etc."
                : "Solo se permite iniciar sesión con Google."}
            </Subtitle>

            {!sessionEmail ? (
              <div className="mt-5 grid gap-4">
                <div className="rounded-2xl border border-white/10 bg-black/20 p-4 text-sm text-white/75">
                  <div className="flex items-center gap-2">
                    <ShieldCheck size={16} className="opacity-80" />
                    <span className="font-medium text-white">Acceso seguro</span>
                  </div>
                  <div className="mt-1 text-xs text-white/60">
                    Tu cuenta queda registrada automáticamente como <b>Joven</b>.
                  </div>
                </div>

                <Button
                  type="button"
                  onClick={signInGoogle}
                  disabled={busy}
                  className="w-full justify-center py-3 text-base"
                >
                  {busy ? "Conectando…" : "Entrar con Google"}
                  <ArrowRight size={18} />
                </Button>

                {msg && (
                  <p className={msg.startsWith("✅") ? "text-sm text-green-300" : "text-sm text-red-300"}>
                    {msg}
                  </p>
                )}
              </div>
            ) : (
              <div className="mt-5 text-sm text-white/70">Navega con el menú superior.</div>
            )}
          </Card>
        </div>
      </PageFade>
    </Container>
  );
}

// "use client";

// import Link from "next/link";
// import { useState } from "react";
// import { supabase } from "@/lib/supabaseClient";
// import { Container, Card, Title, Subtitle, Button, PageFade } from "@/components/ui";
// import { Sparkles, ShieldCheck, ArrowRight } from "lucide-react";
// import { useMyProfile } from "@/lib/useMyProfile";

// function traducirError(msg: string) {
//   const m = (msg ?? "").toLowerCase();
//   if (m.includes("invalid login")) return "Correo o contraseña incorrectos.";
//   if (m.includes("email not confirmed")) return "Tu correo no está confirmado (revisa tu email).";
//   if (m.includes("jwt")) return "Tu sesión expiró. Inicia sesión de nuevo.";
//   if (m.includes("rate limit")) return "Demasiados intentos. Espera un momento y vuelve a intentar.";
//   return "Ocurrió un error. Intenta de nuevo.";
// }

// function ActionCard({
//   href,
//   title,
//   icon,
//   enabled,
//   hint,
// }: {
//   href: string;
//   title: string;
//   icon: string;
//   enabled: boolean;
//   hint?: string;
// }) {
//   const base =
//     "rounded-xl border border-white/10 bg-black/20 p-4 text-sm text-white/80 transition select-none";
//   const enabledCls = "hover:bg-white/10 hover:border-white/15 active:scale-[0.99] cursor-pointer";
//   const disabledCls = "opacity-50 cursor-not-allowed";

//   const content = (
//     <div className={[base, enabled ? enabledCls : disabledCls].join(" ")}>
//       <div className="flex items-center justify-between gap-3">
//         <div className="flex items-center gap-2">
//           <span className="text-base">{icon}</span>
//           <span>{title}</span>
//         </div>
//         <span className="text-white/50">→</span>
//       </div>
//       {!enabled && hint ? <div className="mt-2 text-xs text-white/50">{hint}</div> : null}
//     </div>
//   );

//   if (!enabled) return content;

//   return (
//     <Link href={href} className="block" prefetch>
//       {content}
//     </Link>
//   );
// }

// export default function Home() {
//   const [busy, setBusy] = useState(false);
//   const [msg, setMsg] = useState("");

//   const { loading, session, profile, error } = useMyProfile();
//   const sessionEmail = session?.user?.email ?? null;
//   const hasSession = !!session?.user?.id;

//   const displayName =
//     profile?.name ||
//     (session?.user?.user_metadata?.full_name as string | undefined) ||
//     (session?.user?.user_metadata?.name as string | undefined) ||
//     (sessionEmail ? sessionEmail.split("@")[0] : null);

//   async function signInGoogle() {
//     setMsg("");
//     try {
//       setBusy(true);
//       const { error } = await supabase.auth.signInWithOAuth({
//         provider: "google",
//         options: {
//           redirectTo: window.location.origin + "/auth/callback",
//         },
//       });
//       if (error) setMsg(traducirError(error.message));
//     } finally {
//       setBusy(false);
//     }
//   }

//   const authError = !sessionEmail ? (error || msg) : "";

//   return (
//     <Container>
//       <div className="mb-6 flex items-center gap-4">
//         <div className="h-14 w-14 rounded-3xl bg-white/5 border border-white/10 grid place-items-center text-lg font-semibold">
//           MA
//         </div>
//         <div>
//           <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight">Ministerio Águilas</h1>
//           <p className="text-sm text-white/70">Casa de Dios Cruzada Cristiana</p>
//         </div>
//       </div>

//       <PageFade>
//         <div className="grid gap-6 md:grid-cols-2 items-start">
//           <Card className="relative overflow-hidden">
//             <div className="absolute -top-24 -right-24 h-56 w-56 rounded-full bg-white/10 blur-2xl" />
//             <div className="absolute -bottom-24 -left-24 h-56 w-56 rounded-full bg-white/10 blur-2xl" />

//             <div className="flex items-center gap-2">
//               <Sparkles size={18} className="opacity-80" />
//               <Title>Bienvenido</Title>
//             </div>
//             <Subtitle>Lleva un registro sencillo de tu lectura bíblica y tu tiempo de oración.</Subtitle>

//             {/* ✅ Ahora son “botones”/links */}
//             <div className="mt-5 grid gap-3 text-sm text-white/80">
//               <ActionCard
//                 href="/reporte"
//                 title="Reporte diario"
//                 icon="✅"
//                 enabled={hasSession}
//                 hint="Inicia sesión para abrir Reporte."
//               />
//               <ActionCard
//                 href="/mis-estadisticas"
//                 title="Estadísticas personales"
//                 icon="📊"
//                 enabled={hasSession}
//                 hint="Inicia sesión para ver tus estadísticas."
//               />
//               <div className="rounded-xl border border-white/10 bg-black/20 p-4">
//                 🔒 Acceso por roles
//               </div>
//             </div>

//             {sessionEmail && (
//               <div className="mt-5 text-sm text-white/70">
//                 Sesión iniciada: <strong className="text-white">{sessionEmail}</strong>
//                 {displayName ? (
//                   <div className="mt-1 text-xs text-white/60">
//                     Hola, <span className="text-white/80">{displayName}</span>
//                     {profile?.role ? ` — ${profile.role === "youth" ? "Joven" : profile.role}` : " — Cargando rol…"}
//                   </div>
//                 ) : null}
//               </div>
//             )}

//             {!sessionEmail && loading && (
//               <div className="mt-5 text-xs text-white/50">Verificando sesión…</div>
//             )}
//           </Card>

//           <Card>
//             <Title>{sessionEmail ? "Listo para continuar" : "Iniciar sesión"}</Title>
//             <Subtitle>
//               {sessionEmail
//                 ? "Usa el menú superior para Reporte, Mis estadísticas, etc."
//                 : "Solo se permite iniciar sesión con Google."}
//             </Subtitle>

//             {!sessionEmail ? (
//               <div className="mt-5 grid gap-4">
//                 <div className="rounded-2xl border border-white/10 bg-black/20 p-4 text-sm text-white/75">
//                   <div className="flex items-center gap-2">
//                     <ShieldCheck size={16} className="opacity-80" />
//                     <span className="font-medium text-white">Acceso seguro</span>
//                   </div>
//                   <div className="mt-1 text-xs text-white/60">
//                     Tu cuenta queda registrada automáticamente como <b>Joven</b>.
//                   </div>
//                 </div>

//                 <Button
//                   type="button"
//                   onClick={signInGoogle}
//                   disabled={busy}
//                   className="w-full justify-center py-3 text-base"
//                 >
//                   {busy ? "Conectando…" : "Entrar con Google"}
//                   <ArrowRight size={18} />
//                 </Button>

//                 {authError && (
//                   <p className={authError.startsWith("✅") ? "text-sm text-green-300" : "text-sm text-red-300"}>
//                     {authError}
//                   </p>
//                 )}
//               </div>
//             ) : (
//               <div className="mt-5 text-sm text-white/70">Navega con el menú superior.</div>
//             )}
//           </Card>
//         </div>
//       </PageFade>
//     </Container>
//   );
// }

// // // "use client";

// // // import { useEffect, useState } from "react";
// // // import { supabase } from "@/lib/supabaseClient";
// // // import { Container, Card, Title, Subtitle, Button, PageFade } from "@/components/ui";
// // // import { Sparkles, ShieldCheck, ArrowRight } from "lucide-react";

// // // function traducirError(msg: string) {
// // //   const m = (msg ?? "").toLowerCase();
// // //   if (m.includes("invalid login")) return "Correo o contraseña incorrectos.";
// // //   if (m.includes("email not confirmed")) return "Tu correo no está confirmado (revisa tu email).";
// // //   if (m.includes("jwt")) return "Tu sesión expiró. Inicia sesión de nuevo.";
// // //   if (m.includes("rate limit")) return "Demasiados intentos. Espera un momento y vuelve a intentar.";
// // //   return "Ocurrió un error. Intenta de nuevo.";
// // // }

// // // export default function Home() {
// // //   const [busy, setBusy] = useState(false);

// // //   const [sessionEmail, setSessionEmail] = useState<string | null>(null);
// // //   const [msg, setMsg] = useState("");

// // //   useEffect(() => {
// // //     supabase.auth.getSession().then(({ data }) => {
// // //       setSessionEmail(data.session?.user.email ?? null);
// // //     });

// // //     const { data: sub } = supabase.auth.onAuthStateChange((_event, session) => {
// // //       setSessionEmail(session?.user.email ?? null);
// // //     });

// // //     return () => sub.subscription.unsubscribe();
// // //   }, []);

// // //   async function signInGoogle() {
// // //     setMsg("");
// // //     try {
// // //       setBusy(true);
// // //       const { error } = await supabase.auth.signInWithOAuth({
// // //         provider: "google",
// // //         options: {
// // //           redirectTo: window.location.origin + "/auth/callback",
// // //         },
// // //       });
// // //       if (error) setMsg(traducirError(error.message));
// // //     } finally {
// // //       setBusy(false);
// // //     }
// // //   }

// // //   return (
// // //     <Container>
// // //       <div className="mb-6 flex items-center gap-4">
// // //         <div className="h-14 w-14 rounded-3xl bg-white/5 border border-white/10 grid place-items-center text-lg font-semibold">MA</div>
// // //         <div>
// // //           <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight">Ministerio Águilas</h1>
// // //           <p className="text-sm text-white/70">Casa de Dios Cruzada Cristiana</p>
// // //         </div>
// // //       </div>

// // //       <PageFade>
// // //         <div className="grid gap-6 md:grid-cols-2 items-start">
// // //           <Card className="relative overflow-hidden">
// // //             <div className="absolute -top-24 -right-24 h-56 w-56 rounded-full bg-white/10 blur-2xl" />
// // //             <div className="absolute -bottom-24 -left-24 h-56 w-56 rounded-full bg-white/10 blur-2xl" />

// // //             <div className="flex items-center gap-2">
// // //               <Sparkles size={18} className="opacity-80" />
// // //               <Title>Bienvenido</Title>
// // //             </div>
// // //             <Subtitle>
// // //               Lleva un registro sencillo de tu lectura bíblica y tu tiempo de oración.
// // //             </Subtitle>

// // //             <div className="mt-5 grid gap-3 text-sm text-white/80">
// // //               <div className="rounded-xl border border-white/10 bg-black/20 p-4">
// // //                 ✅ Reporte diario
// // //               </div>
// // //               <div className="rounded-xl border border-white/10 bg-black/20 p-4">
// // //                 📊 Estadísticas personales
// // //               </div>
// // //               <div className="rounded-xl border border-white/10 bg-black/20 p-4">
// // //                 🔒 Acceso por roles
// // //               </div>
// // //             </div>

// // //             {sessionEmail && (
// // //               <div className="mt-5 text-sm text-white/70">
// // //                 Sesión iniciada: <strong className="text-white">{sessionEmail}</strong>
// // //               </div>
// // //             )}
// // //           </Card>

// // //           <Card>
// // //             <Title>{sessionEmail ? "Listo para continuar" : "Iniciar sesión"}</Title>
// // //             <Subtitle>
// // //               {sessionEmail
// // //                 ? "Usa el menú superior para Reporte, Mis estadísticas, Público, etc."
// // //                 : "Solo se permite iniciar sesión con Google."}
// // //             </Subtitle>

// // //             {!sessionEmail ? (
// // //               <div className="mt-5 grid gap-4">
// // //                 <div className="rounded-2xl border border-white/10 bg-black/20 p-4 text-sm text-white/75">
// // //                   <div className="flex items-center gap-2">
// // //                     <ShieldCheck size={16} className="opacity-80" />
// // //                     <span className="font-medium text-white">Acceso seguro</span>
// // //                   </div>
// // //                   <div className="mt-1 text-xs text-white/60">
// // //                     Tu cuenta queda registrada automáticamente como <b>Joven</b>.
// // //                   </div>
// // //                 </div>

// // //                 <Button type="button" onClick={signInGoogle} disabled={busy} className="w-full justify-center py-3 text-base">
// // //                   {busy ? "Conectando…" : "Entrar con Google"}
// // //                   <ArrowRight size={18} />
// // //                 </Button>

// // //                 {msg && (
// // //                   <p className={msg.startsWith("✅") ? "text-sm text-green-300" : "text-sm text-red-300"}>
// // //                     {msg}
// // //                   </p>
// // //                 )}
// // //               </div>
// // //             ) : (
// // //               <div className="mt-5 text-sm text-white/70">
// // //                 Navega con el menú superior.
// // //               </div>
// // //             )}
// // //           </Card>
// // //         </div>
// // //       </PageFade>
// // //     </Container>
// // //   );
// // // }


// // "use client";

// // import { useState } from "react";
// // import { supabase } from "@/lib/supabaseClient";
// // import { Container, Card, Title, Subtitle, Button, PageFade } from "@/components/ui";
// // import { Sparkles, ShieldCheck, ArrowRight } from "lucide-react";
// // import { useMyProfile } from "@/lib/useMyProfile";

// // function traducirError(msg: string) {
// //   const m = (msg ?? "").toLowerCase();
// //   if (m.includes("invalid login")) return "Correo o contraseña incorrectos.";
// //   if (m.includes("email not confirmed")) return "Tu correo no está confirmado (revisa tu email).";
// //   if (m.includes("jwt")) return "Tu sesión expiró. Inicia sesión de nuevo.";
// //   if (m.includes("rate limit")) return "Demasiados intentos. Espera un momento y vuelve a intentar.";
// //   return "Ocurrió un error. Intenta de nuevo.";
// // }

// // export default function Home() {
// //   const [busy, setBusy] = useState(false);
// //   const [msg, setMsg] = useState("");

// //   // ✅ Usa el AuthProvider (una sola fuente de verdad)
// //   const { loading, session, profile, error } = useMyProfile();

// //   const sessionEmail = session?.user?.email ?? null;
// //   const displayName =
// //     profile?.name ||
// //     (session?.user?.user_metadata?.full_name as string | undefined) ||
// //     (session?.user?.user_metadata?.name as string | undefined) ||
// //     (sessionEmail ? sessionEmail.split("@")[0] : null);

// //   async function signInGoogle() {
// //     setMsg("");
// //     try {
// //       setBusy(true);
// //       const { error } = await supabase.auth.signInWithOAuth({
// //         provider: "google",
// //         options: {
// //           redirectTo: window.location.origin + "/auth/callback",
// //         },
// //       });
// //       if (error) setMsg(traducirError(error.message));
// //     } finally {
// //       setBusy(false);
// //     }
// //   }

// //   // Si hay error de auth real (y no hay sesión), muéstralo aquí
// //   const authError = !sessionEmail ? (error || msg) : "";

// //   return (
// //     <Container>
// //       <div className="mb-6 flex items-center gap-4">
// //         <div className="h-14 w-14 rounded-3xl bg-white/5 border border-white/10 grid place-items-center text-lg font-semibold">
// //           MA
// //         </div>
// //         <div>
// //           <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight">Ministerio Águilas</h1>
// //           <p className="text-sm text-white/70">Casa de Dios Cruzada Cristiana</p>
// //         </div>
// //       </div>

// //       <PageFade>
// //         <div className="grid gap-6 md:grid-cols-2 items-start">
// //           <Card className="relative overflow-hidden">
// //             <div className="absolute -top-24 -right-24 h-56 w-56 rounded-full bg-white/10 blur-2xl" />
// //             <div className="absolute -bottom-24 -left-24 h-56 w-56 rounded-full bg-white/10 blur-2xl" />

// //             <div className="flex items-center gap-2">
// //               <Sparkles size={18} className="opacity-80" />
// //               <Title>Bienvenido</Title>
// //             </div>
// //             <Subtitle>Lleva un registro sencillo de tu lectura bíblica y tu tiempo de oración.</Subtitle>

// //             <div className="mt-5 grid gap-3 text-sm text-white/80">
// //               <div className="rounded-xl border border-white/10 bg-black/20 p-4">✅ Reporte diario</div>
// //               <div className="rounded-xl border border-white/10 bg-black/20 p-4">📊 Estadísticas personales</div>
// //               <div className="rounded-xl border border-white/10 bg-black/20 p-4">🔒 Acceso por roles</div>
// //             </div>

// //             {sessionEmail && (
// //               <div className="mt-5 text-sm text-white/70">
// //                 Sesión iniciada: <strong className="text-white">{sessionEmail}</strong>
// //                 {displayName ? (
// //                   <div className="mt-1 text-xs text-white/60">
// //                     Hola, <span className="text-white/80">{displayName}</span>
// //                     {profile?.role ? ` — ${profile.role === "youth" ? "Joven" : profile.role}` : " — Cargando rol…"}
// //                   </div>
// //                 ) : null}
// //               </div>
// //             )}

// //             {!sessionEmail && loading && (
// //               <div className="mt-5 text-xs text-white/50">Verificando sesión…</div>
// //             )}
// //           </Card>

// //           <Card>
// //             <Title>{sessionEmail ? "Listo para continuar" : "Iniciar sesión"}</Title>
// //             <Subtitle>
// //               {sessionEmail
// //                 ? "Usa el menú superior para Reporte, Mis estadísticas, etc."
// //                 : "Solo se permite iniciar sesión con Google."}
// //             </Subtitle>

// //             {!sessionEmail ? (
// //               <div className="mt-5 grid gap-4">
// //                 <div className="rounded-2xl border border-white/10 bg-black/20 p-4 text-sm text-white/75">
// //                   <div className="flex items-center gap-2">
// //                     <ShieldCheck size={16} className="opacity-80" />
// //                     <span className="font-medium text-white">Acceso seguro</span>
// //                   </div>
// //                   <div className="mt-1 text-xs text-white/60">
// //                     Tu cuenta queda registrada automáticamente como <b>Joven</b>.
// //                   </div>
// //                 </div>

// //                 <Button
// //                   type="button"
// //                   onClick={signInGoogle}
// //                   disabled={busy}
// //                   className="w-full justify-center py-3 text-base"
// //                 >
// //                   {busy ? "Conectando…" : "Entrar con Google"}
// //                   <ArrowRight size={18} />
// //                 </Button>

// //                 {authError && (
// //                   <p className={authError.startsWith("✅") ? "text-sm text-green-300" : "text-sm text-red-300"}>
// //                     {authError}
// //                   </p>
// //                 )}
// //               </div>
// //             ) : (
// //               <div className="mt-5 text-sm text-white/70">
// //                 Navega con el menú superior.
// //               </div>
// //             )}
// //           </Card>
// //         </div>
// //       </PageFade>
// //     </Container>
// //   );
// // }
